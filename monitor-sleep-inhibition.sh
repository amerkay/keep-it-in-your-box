...
QDBUS=""
for c in qdbus6 qdbus-qt6 qdbus; do command -v "$c" >/dev/null 2>&1 && { QDBUS="$c"; break; }; done
...
input_capture_start(){   # launch background readers; returns immediately
  ...
  INPUT_TMP=$(mktemp -d 2>/dev/null) || { INPUT_TMP=""; return; }
  local e
...
input_capture_report(){  # wait for readers, name devices that fired
  if [ ${#INPUT_DEVS[@]} -eq 0 ]; then
    echo "n/a (no readable /dev/input — run: sudo usermod -aG input $USER; then re-login)"; return
  fi
  [ ${#INPUT_PIDS[@]} -gt 0 ] && wait "${INPUT_PIDS[@]}" 2>/dev/null
  local f n nm out=""
  for f in "$INPUT_TMP"/event*; do
    [ -f "$f" ] || continue
    n=$(cat "$f" 2>/dev/null); [ "${n:-0}" -gt 0 ] 2>/dev/null || continue
...
cleanup(){ say ""; say "=== stopped $(date '+%F %T') ==="; kill "$JPID" 2>/dev/null; wait "$JPID" 2>/dev/null; exit 0; }
...
while true; do
  ...
  ims=$(kde_idle_ms); ims=${ims//[^0-9]/}; ss=$(( 10#${ims:-0} / 1000 ))   # screensaver clock (dead on Wayland)
  kde="$(kde_inhibitions | tr -d '\n' | sed 's/  */ /g')"
  [ -z "${kde// }" ] && kde="(none)"
  ac=$(ac_online); lid=$(lid_state); susp=$(susp_count)
  ...
  while read -r ip; do
    ...
    [ -n "${guard:-}" ] || continue
    ct=$(guard_arg "$guard" head); tg=$(guard_arg "$guard" tail)
  ...
  for tg in "${!GTAG_CONT[@]}"; do
    while read -r cpid cbytes; do
      [ -n "$cpid" ] || continue
      prev="${CIO1["$tg,$cpid"]:-}"; [ -n "$prev" ] || continue   # only pids in BOTH samples
      d=$(( cbytes - prev )); (( d < 0 )) && continue
      if (( d > ${TAGMAX[$tg]:-0} )); then TAGMAX[$tg]=$d; TAGPID[$tg]=$cpid; fi
    done <<< "$(sample_container_wchar "${GTAG_CONT[$tg]}" "$tg")"
  done
  ...
  if [ ${#INPUT_DEVS[@]} -gt 0 ]; then
    ...
    is=$(( now - last_input_ts )); isrc="no-input-for"
  else
    is=$hint_idle; isrc="loginhint"
  fi

  # global top writers (host-side): names a chatty process system-wide for context
  tops=""
  for pid in "${!IO2[@]}"; do
    w1=${IO1[$pid]:-}; [ -n "$w1" ] || continue      # only pids in BOTH samples
    d=$(( ${IO2[$pid]} - w1 )); (( d < 0 )) && continue
    (( d > 0 )) && tops+="$d $pid"$'\n'
  done
  ...
  if [ -n "$cc_pids" ]; then
    while read -r ip; do
      ...
      gtag=$(guard_arg "${guard:-0}" tail); gcont=$(guard_arg "${guard:-0}" head)
      gmin=$(env_val "${guard:-0}" SLEEP_GUARD_MIN_BYTES); gmin=${gmin:-$MIN_BYTES}
      orphan=""
      ...
      mx=0; mxpid=""; mxcmd=""
      if [ -n "$gtag" ]; then mx=${TAGMAX[$gtag]:-0}; mxpid=${TAGPID[$gtag]:-}; fi
      [ -n "$mxpid" ] && [ -n "$gcont" ] && mxcmd=$(container_cmd "$gcont" "$mxpid")
      verdict="justified"; (( mx < gmin )) && verdict="UNJUSTIFIED (max ${mx} < ${gmin}) — riding 30s grace tail or stuck"
  ...
  if [ ${#TAGMAX[@]} -gt 0 ]; then
    for tag in "${!TAGMAX[@]}"; do
      _cmd=""; [ -n "${TAGPID[$tag]:-}" ] && _cmd=$(container_cmd "${GTAG_CONT[$tag]:-}" "${TAGPID[$tag]}")
  ...
  rest=$(( INTERVAL - SUBSAMPLE )); (( rest > 0 )) && sleep "$rest"
done
