# Sandbox rules — every sandbox session
...
- **Redacted** (`.kibignore`, `.env` and `.env.*`) — reads return a stub, writes fail EACCES. Need a
...
- **Shared assets** (`~/.claude-shared/hooks/`, `plugins/`, `skills/`, `agents/`, `commands/`)
  ...
  repo would pivot into all of them. (This CLAUDE.md is *not* one of them: kib assembles it into
  ...
  Per-project still works. To make this shared, close every session for
  this project and relaunch with:  kib unlock-shared
  ```

On EACCES, or on a launch refused by the host's audit gate: stop and report it verbatim, including what you were attempting. Do not retry via another path, another tool, a different config scope (`--global`, `--system`), or by asking the user to disable the guard. A legitimate need is a conversation, not a workaround.
...
The launcher pins `leftArrowOpensAgents: false` into `.claude.json` on every launch, so **`←` does nothing** in a sandbox session. Do not re-enable it, and never tell the user to press `←` to reach the agent list.
...
Comments: compact — two lines, not ten. Give the constraint, the non-obvious *why*, or the failure mode. Never restate the code, narrate how a bug was found, or repeat a rationale that lives elsewhere; long stories belong in a design note, linked.