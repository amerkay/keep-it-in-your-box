...
- **Stale-rules refusal:** redaction is fixed at container creation, so `cc` refuses to attach if `.kibignore` changed since the container started, or if the running container has no sidecar. Same pattern for the `--unlock-shared` mode mismatch.
