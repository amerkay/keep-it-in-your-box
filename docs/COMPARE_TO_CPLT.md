...
| **Auto-update integrity** | ⚠️ Background image rebuild (`tools/build-bg.sh`), version pinned by build arg; trust = Docker/NodeSource/official installer chain | ⚠️ `cplt update`: mandatory SHA256, `--proto-redir=https`, atomic rename; **no GPG/Sigstore sig** (accepted); prefers Homebrew | ➖ tie — neither cryptographically signs |
...
| **Operational robustness** | ✅ `host/sleep-guard.sh` (busiest-process sampling, proactive lid-shut suspend), live-DNS sync, one-container-per-project lifecycle | ➖ Not in scope | **cc** (out of cplt's scope) |
