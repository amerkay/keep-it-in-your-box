...
| **`host/sleep-guard.sh` injection** | Blocked — at audit time the one container-originated value reaching a bash-arithmetic sink was sanitized by an `awk '$NF + 0'` coercion in the `wchar` sampler; container and session names reached host command lines only as safely-quoted args. **Superseded, and the sink is now gone:** the sampler was deleted with the move to hook state, so no container-originated value reaches host arithmetic at all. `host/sleep-state.sh` tests marker EXISTENCE only — it never opens a file in the container-writable tree, so there is no content to coerce — and rejects a session dir that is a symlink. Nothing to keep; the guarantee is structural rather than a coercion to preserve. (`sleep-guard.md`) |
