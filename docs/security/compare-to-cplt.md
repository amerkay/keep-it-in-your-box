...
| **`.vscode` / IDE auto-exec** | ✅ `[protect]` set (`.vscode/`, `.devcontainer/`, `.envrc`) — writes `EPERM`; `.idea/` withdrawn 2026-07-30 (it broke `pnpm install`) | ❌ explicitly out of scope: "IDE trust boundary." Mitigation = review `git diff` | **kib** |
| **Guard immune to project override** | ✅ `[protect]` outranks project negations (`_verdict` tallies them separately); `[redact]` is opt-out-able per file (`!.env`, printed at each launch) — the user's own secrets, the user's call; config read from the FUSE view | ✅ `.cplt.toml` `[propose]` inert until `cplt trust accept`; config read from `git HEAD`, content-pinned SHA-256 approvals | ➖ tie — both defeat self-relaxation cleanly |
