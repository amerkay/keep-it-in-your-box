...
- **`~/.claude-shared/` and `$CLAUDE_CONFIG_DIR` are LIVE host state** (bind mounts). Never aim
  destructive or migration logic at them — build a fake `$HOME` in the scratchpad;
  `CC_MIGRATE_TEST=1` disables `migrate-sessions.sh`'s safety checks for exactly that.
...
Image build (host-side): `docker build -t keep-it-in-your-box .` — args `NODE_MAJOR`,
`CLAUDE_VERSION`.
...
- **Never reintroduce a broker credential refresh loop; never broker `.credentials.json`** —
  ...
  every provider. (`credential-broker.md`)
