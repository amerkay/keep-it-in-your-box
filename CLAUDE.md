...
- **`cc`** — Host-side launcher script. Builds the image if missing, then `docker run` with volume mounts, seccomp profile, and environment forwarding. Run from any project directory: `./cc` (or with args passed through to the container CMD).
- **`Dockerfile`** — Debian bookworm image with Node.js 20, Python 3, dev tools, and Claude Code installed via official installer. Supports `CUSTOM_PACKAGES` build arg. Use `CACHE_BUST` arg to force Claude Code reinstall.
