...
cc --login dataforseo        # paste the base64 of 'login:password'
```

`cc --login` stores the secret host-side (mode 600); the next `cc` launch injects a
...
- **`dataforseo.json`** — `reverse_proxy_mcp`: a remote MCP (`https://mcp.dataforseo.com/http`)
  ...
  re-originates TLS; the agent only ever sees `http://cc-broker:<port>/http`.
...
- **`gsc.json`** — `hosted_mcp`: [mcp-gsc](https://github.com/AminForou/mcp-gsc) reads a Google
  ...
  reaches it over the broker network. Credential: `cc --login gsc` and give the path to your
  service-account JSON.

## Equivalent without copying a file

`cc --add-mcp` writes the same definition from flags:

```bash
# reverse_proxy_mcp (remote, static header)
cc --add-mcp dataforseo --url https://mcp.dataforseo.com/http --header "Authorization: Basic"
...
Ports at/above 8100 are the user band; `cc --add-mcp` auto-assigns the next free one. The
