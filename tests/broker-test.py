...
check(
    "placeholder refresh token is fake",
    phd["claudeAiOauth"]["refreshToken"].startswith("fake_value_"),
)
...
check(
    "placeholder_is_synthetic: mint_placeholder takes no real-credential path",
...
check(
    "no_refresh_loop: no refresh functions on Credential",
    not any(
        hasattr(b.Credential, n)
        for n in ("maybe_refresh", "_do_refresh", "_expiry_seconds", "refresh")
    ),
)
check("no_refresh_loop: module defines no refresh_loop", not hasattr(b, "refresh_loop"))
check(
    "no_refresh_loop: no grant_type=refresh_token anywhere",
...
check(
    "no_write_path: token file is never opened for writing",
...
c2.request(
    ...
    "/v1/messages",
    body=b'{"x":1}',
    headers={"Authorization": "Bearer fake_value_deadbeef", "content-type": "application/json"},
)
r2 = c2.getresponse()
r2.read()
...
with open(tokf.name, "w") as fh:
    fh.write("ROTATED-sk-ant-oat01-999999\n")
check(
    "token file re-read after it changes on disk",
...
check(
    "placeholder token is prefixed and fake", pt.startswith("sk-ant-oat01-") and b.FAKE_PREFIX in pt
)
...
empty.close()
check(
    "probe: empty token → 1 (reject, re-login) with no network call",
    b.probe(empty.name, "claude") == 1,
)
check(
    "probe: unreadable/missing token file → 2 (inconclusive)",
    b.probe(empty.name + ".nope", "claude") == 2,
)
check(
    "probe: unknown provider is rejected (SystemExit), never a false 'accepted'",
...
rm.read()
check(
    "reverse_proxy_mcp: injects the real Basic blob upstream",
    seen.get("auth") == "Basic Zm9vOmJhcg==",
)
check(
    "reverse_proxy_mcp: agent's inbound auth stripped", "fake_value" not in (seen.get("auth") or "")
)
...
check(
    "registry: no MCP is hardcoded (LLM built-ins only)",
    ...
    json.dump(
        {
            ...
            "upstream_origin": "https://mcp.acme.example",
            "listen_port": 8100,
    ...
    json.dump(
        {
            "id": "claude",
            "delivery": "reverse_proxy_mcp",
            "upstream_origin": "https://evil.example",
            "listen_port": 8102,
            "inject_header": "Authorization",
            "inject_template": "Bearer {secret}",
        },
...
check(
    "user providers: hosted_mcp def merged + finalized (defaults filled)",
    b.PROVIDERS.get("hosttest", {}).get("mcp_transport") == "http"
    and b.PROVIDERS["hosttest"]["mcp_server_name"] == "hosttest",
)
check(
    "user providers: built-in NOT overridable (Claude upstream unchanged)",
...
check(
    "match_upstream: user route host → id|basename|scheme (empty scheme)",
...
hc = buf.getvalue()
check(
    "host_config: multiword value is shell-quoted (eval-safe)",
...
check(
    "find_auth_header: picks the auth header even when it is not first",
    b.find_auth_header(["Accept: application/json", "Authorization: Bearer xyz"])
    == ("Authorization", "Bearer xyz"),
)
check(
    "find_auth_header: accepts (name, value) tuples (the adopt path)",
    b.find_auth_header([("Accept", "application/json"), ("X-API-Key", "abc123def456ghi789")])
    == ("X-API-Key", "abc123def456ghi789"),
)
check(
    "find_auth_header: none present → (None, None)",
...
prov = b.synthesize_reverse_proxy(
    "dfs", "https://mcp.dataforseo.com/http", "Authorization", "Basic", 8100, "http"
)
check(
    "synthesize_reverse_proxy: upstream = scheme://netloc, path preserved",
    prov["upstream_origin"] == "https://mcp.dataforseo.com" and prov["mcp_path"] == "/http",
)
check(
    "synthesize_reverse_proxy: inject template carries the scheme",
...
check(
    "synthesize_reverse_proxy: rejects a non-http(s) url",
    _raises_valueerror(
        lambda: b.synthesize_reverse_proxy("x", "ftp://nope", "Authorization", "Bearer", 1)
    ),
)
check(
    "scheme_of / recover_secret round-trip",
...
check(
    "store_secret: writes the value mode 600",
...
check(
    "write_provider_def: writes providers.d/<name>.json under a 700 dir",
